...
st.set_page_config(page_title="V23 FULL AI", page_icon="⚽", layout="wide")
...
def parse_goal7(html):
    ...
    for tr in soup.find_all("tr"):
        ...
        if len(tds) < 6:
            if any(k in txt.lower() for k in ["league", "liga", "cup", "bundesliga", "serie", "premier"]):
                league = txt
            continue
...
def score_ai(row):
    ...
    if ah < 0:
        home_score += abs(ah) * 10
    elif ah > 0:
        away_score += abs(ah) * 10

    home_score += random.uniform(0, 8)
    away_score += random.uniform(0, 8)
    ...
    ou = "ສູງ" if len(nums) % 2 == 0 else "ຕ່ຳ"

    value = conf
    if abs(ah) >= 1:
        value += 10
    if "ต่อ" in raw or "รอง" in raw:
        value += 8
    if conf > 60:
        value += 10
    if nums:
...
st.title("⚽ V23 FULL AI + REAL SIGNAL")
st.caption("ດຶງຕາຕະລາງ goal7 + ຊື່ທີມ + AH + OU + Value Bet + Alert")
...
if st.button("🚀 ດຶງຂໍ້ມູນ V23", use_container_width=True):
    ...
    st.subheader("🔥 5 ຄູ່ເດັດ V23")
    st.dataframe(final.head(5), use_container_width=True)

    st.subheader("🚨 Alert")
    alerts = final[final["Alert"] != "—"]
